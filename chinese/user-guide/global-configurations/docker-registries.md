...
|**注册表类型** |从下拉列表中选择注册表类型：<br><ul><li>[ECR](#注册表类型ecr)</li></ul><ul><li>[Docker](#登记类别docker)</li></ul><ul><li>[Azure](#注册表类型azure)</li></ul><ul><li>[工件注册表（GCP）](#注册表类型工件注册表gcp)</li></ul><ul><li>[GCR](#注册表类型google-container-registrygcr)</li></ul><ul><li>[Quay](#注册表类型quay)</li></ul><ul><li>[其他](#注册表类型其他)</li></ul>`注意`：每个**注册表类型**的凭据输入字段都不同。|
|**注册表URL**|提供您的注册表的URL。|
