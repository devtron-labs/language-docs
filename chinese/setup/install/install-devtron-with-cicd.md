...
在本节中，我们将详细介绍如何安装带有CI/CD的Devtron的步骤。
...
运行以下命令以安装最新版本的Devtron以及CI/CD模块：
~~~ bash
...
**注意事项**：如果要在安装过程中配置Blob存储，请参阅 [在安装时配置Blob存储](#在安装时配置blob存储).

## 如果需要，请安装AWS EBS CSI驱动程序

如果您使用1.23或更高版本的EKS，则必须安装[aws-ebs-csi-driver](https://docs.aws.amazon.com/eks/latest/userguide/ebs-csi.html)。

运行以下命令以使用Helm安装AWS EBS CSI驱动程序：
~~~ bash
...
* 要安装特定版本的Devtron，请在安装指令后加上`--set installer.release="vX.X.X"`，其中[vx.x.x](https://github.com/devtron-labs/devtron/releases)为`发布标签`。
