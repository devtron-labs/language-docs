# Installer Devtron avec CICD
Dans cette section, nous décrivons en détail les étapes de l'installation de Devtron avec l'intégration CI/CD.

## Avant de commencer

Installez [Helm](https://helm.sh/docs/intro/install/), si vous ne l'avez pas installé.

## Installer Devtron avec CI/CD

Exécutez la commande suivante pour installer la dernière version de Devtron avec le module CI/CD :
~~~ bash
...
**Note** : Si vous souhaitez configurer le stockage Blob pendant l'installation, reportez-vous à [configurer le stockage blob pendant l'installation](#configurer-le-stockage-blob-après-linstallation).

## Installez le pilote AWS EBS CSI, si nécessaire

Si vous utilisez EKS version 1.23 ou ultérieure, vous devez installer [aws-ebs-csi-driver](https://docs.aws.amazon.com/eks/latest/userguide/ebs-csi.html).

Exécutez la commande suivante pour installer le pilote aws ebs csi à l'aide de Helm :
~~~ bash
...
* Pour installer une version particulière de Devtron où `vx.x.x` est le [tag de la version](https://github.com/devtron-labs/devtron/releases), ajoutez à la commande d'installation `--set installer.release="vX.X.X"`.
